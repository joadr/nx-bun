...
import { resolveEntryFromBuildTarget } from "../src/executors/run/executor";
...
test("validateOptions rejects ambiguous options", () => {
  expect(() => validateOptions({})).toThrow(
    'Provide either "script", "entry", or "buildTarget".',
  );
  expect(() =>
    validateOptions({ script: "dev", entry: "src/main.ts" }),
  ).toThrow('Only one of "script" or "entry" may be provided.');
});
...
test("buildCommandArguments preserves bun invocation shape", () => {
  ...
  ).toEqual(["--inspect", "src/main.ts", "--foo"]);
...
test("resolveWorkingDirectory rejects missing directories", () => {
  ...
  expect(() =>
    resolveWorkingDirectory({ cwd: "./definitely-missing" }, context),
  ).toThrow("The working directory does not exist:");
});

test("mergeEnvironment and bun binary resolution are explicit", () => {
  expect(resolveBunBinary({ bunPath: "/opt/bun/bin/bun" })).toBe(
    "/opt/bun/bin/bun",
  );
