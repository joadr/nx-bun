...
type ProjectGraphLike = {
    ...
    externalNodes?: Record<string, { data?: { packageName?: string; version?: string } }>;
};
...
function sortObjectByKeys(value: Record<string, string>): Record<string, string> {
...
function resolveDependencyVersion(rootPackageJson: PackageJson, packageName: string, versionHint?: string): string | undefined {
    return (
...
function collectExternalDependencies(projectName: string, projectGraph: ProjectGraphLike): Map<string, string> {
    ...
    const visit = (name: string): void => {
        ...
        for (const dependency of projectGraph.dependencies?.[name] ?? []) {
            ...
            if (externalNode?.data?.packageName && externalNode.data.version) {
                collected.set(externalNode.data.packageName, externalNode.data.version);
                continue;
            }
...
function collectExplicitExternalDependencies(optionsExternal: string[] | undefined): string[] {
...
export function writeGeneratedPackageJson(options: {
    ...
    const projectGraph = (options.context as { projectGraph?: ProjectGraphLike }).projectGraph;

    const runtimeDependencies = new Map<string, string>();

    if (options.context.projectName && projectGraph) {
        for (const [packageName, version] of collectExternalDependencies(options.context.projectName, projectGraph)) {
            runtimeDependencies.set(packageName, version);
        }
    } else {
        for (const [packageName, version] of Object.entries(rootPackageJson.dependencies ?? {})) {
            runtimeDependencies.set(packageName, version);
        }
    }

    for (const external of collectExplicitExternalDependencies(options.external)) {
        const version = resolveDependencyVersion(rootPackageJson, external);

        if (isNonEmptyString(version)) {
            runtimeDependencies.set(external, version);
        }
    }

    const dependencies = sortObjectByKeys(Object.fromEntries(runtimeDependencies));
    const generatedPackageJson: PackageJson = {
        ...
        ...(rootPackageJson.private !== undefined ? { private: rootPackageJson.private } : {}),
